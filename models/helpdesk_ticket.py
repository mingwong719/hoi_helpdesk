...
class HelpdeskTicket(models.Model):
    ...
    def message_new(self, msg_dict, custom_values=None):
                ...
                if ',' in msg_dict.get('to'):
                    ...
                    if email_to:
                        for to_email in email_to:
                            ...
                            email_address = email_escape_char(email_split(email_address)[0])
                    ...
                    email_address = email_escape_char(email_split(msg_dict.get('to'))[0])
                    if get_ourgoing_server_all and email_address.lower() not in get_ourgoing_server_all.mapped('smtp_user'):
                        partner_id = self.env['res.partner'].search([
                                ('email', '=', email_address)
                            ], limit=1)
        ...
        if 'cc' in msg_dict and msg_dict.get('cc') != '':
                if ',' in msg_dict.get('cc'):
                    ...
                    email_address = email_escape_char(email_split(msg_dict.get('cc'))[0])
                    if get_ourgoing_server_all and email_address.lower() not in get_ourgoing_server_all.mapped('smtp_user'):
                        partner_id = self.env['res.partner'].search([
                                ('email', '=', email_address)
                            ], limit=1)
        ...
        if custom_values.get('team_id'):
            team_id = self.env['helpdesk.team'].sudo().browse(custom_values.get('team_id'))
            if team_id:
                defaults.update({
                    'team_id':team_id.id,
    ...
    def write(self, vals):
        if vals.get('attachment_ids'):
            attachment_ids = self.env['ir.attachment'].sudo().browse(vals.get('attachment_ids')[0][2]).filtered(lambda x:not x.public)
            if attachment_ids:
